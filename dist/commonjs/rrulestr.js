...
const rruleset_js_1 = require("./rruleset.js");
...
function parseInput(s, options) {
    ...
    const exrulevals = [];
    let exdatevals = [];
    const parsedDtstart = (0, parsestring_js_1.parseDtstart)(s);
    ...
    lines.forEach((line) => {
        ...
        switch (name.toUpperCase()) {
            case 'RRULE':
                ...
                rrulevals.push((0, parsestring_js_1.parseString)(line));
                break;
            case 'RDATE':
            ...
            case 'EXRULE':
                ...
                exrulevals.push((0, parsestring_js_1.parseString)(value));
                break;
            case 'EXDATE':
...
function buildRule(s, options) {
    ...
    if (options.forceset ||
        ...
        exrulevals.length ||
        exdatevals.length) {
        const rset = new rruleset_js_1.RRuleSet(noCache);
        rset.dtstart(dtstart);
        rset.tzid(tzid || undefined);
        rrulevals.forEach((val) => {
            rset.rrule(new rrule_js_1.RRule(groomRruleOptions(val, dtstart, tzid), noCache));
        });
        rdatevals.forEach((date) => {
            rset.rdate(date);
        });
        exrulevals.forEach((val) => {
            rset.exrule(new rrule_js_1.RRule(groomRruleOptions(val, dtstart, tzid), noCache));
        });
    ...
    return new rrule_js_1.RRule(groomRruleOptions(val, val.dtstart || options.dtstart || dtstart, val.tzid || options.tzid || tzid), noCache);
}
...
function initializeOptions(options) {
    ...
    keys.forEach(function (key) {
        if (!(0, helpers_js_1.includes)(defaultKeys, key))
            invalid.push(key);
    });
    if (invalid.length) {
...
function extractName(line) {
    ...
    const [name, value] = (0, helpers_js_1.split)(line, ':', 1);
    return {
        name,
        value,
    };
...
function parseRDate(rdateval, parms) {
    ...
    return rdateval.split(',').map((datestr) => (0, dateutil_js_1.untilStringToDate)(datestr));
}
